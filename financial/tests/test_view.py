...
class FinancialTestCase(TestCase):
    ...
    def test_contract_not_super_user(self):
        ...
        response = client.get(
            "/financial/payment/new/", data={"page": 1, "page_size": 5}
        )
        self.assertEqual(response.status_code, 403)
        ...
        response = client.get(
            "/financial/payment/1/save", data={"page": 1, "page_size": 5}
        )
        self.assertEqual(response.status_code, 403)

        response = client.get(
            "/financial/payment/1/payoff", data={"page": 1, "page_size": 5}
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_payment_list(self):
        ...
        response = self.client.get(
            "/financial/payment/", data={"page": 1, "page_size": 5}
        )
        response_body = json.loads(response.content)
