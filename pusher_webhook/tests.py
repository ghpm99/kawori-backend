...
class PusherWebhookViewsRegressionTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_superuser(
            username="pusher-reg", email="pusher-reg@test.com", password="123"
        )
    ...
    def test_pusher_webhook_delegates_to_lib(self):
        ...
        with patch(
            "pusher_webhook.views.pusher.webhook", return_value=HttpResponse("ok")
        ) as mocked_webhook:
    ...
    def test_pusher_auth_parses_body_and_calls_lib_auth(self):
        ...
        with patch(
            "pusher_webhook.views.pusher.auth", return_value=HttpResponse("auth-ok")
        ) as mocked_auth:
